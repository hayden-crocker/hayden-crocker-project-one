// Create a scope for when the user submits a form
const submitForm = {};
// When a user submits a comment in the form it needs to be displayed on the page underneath the previous comment.

// Grab the form element
submitForm.formElement = document.querySelector('form');

// Add an event listener for when the user submits the form
submitForm.updateComment = function () {
    submitForm.formElement.addEventListener('submit', function (e) {
        e.preventDefault();
        ...
        if (nameTask, commentTask) {
            ...
            commentArticle.classList = 'comment';

            const userImage = document.createElement('img');
            userImage.src = "http://placekitten.com/200/200"
            commentArticle.appendChild(userImage);

            const commentText = document.createElement('div');
            commentText.classList = 'commentText';
...
// make sure the content of the form loads before being able to submit the comment
submitForm.setupEventListeners = function () {
    submitForm.updateComment();
}

submitForm.init = function(){
    submitForm.formElement = document.querySelector('form');
    submitForm.setupEventListeners();
}
